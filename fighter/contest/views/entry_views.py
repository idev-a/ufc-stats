...
def _count_entries(fighter_id, selections):
    ...
    for selection in selections:
        if selection.survivor1 and selection.survivor1.id == fighter_id:
            survivor1s.append(
                [selection.survivor1.name, f"{selection.entry.user.displayname}"]
            )
    ...
    for selection in selections:
        if selection.survivor2 and selection.survivor2.id == fighter_id:
            survivor2s.append(
                [selection.survivor2.name, f"{selection.entry.user.displayname}"]
            )
...
def _calc_suv_win_loss(survivor1, survivor2, winner, loser):
    wins = 0
    losses = 0
    if survivor1.get("id") == winner.get("id") or survivor2.get("id") == winner.get(
        "id"
    ):
        wins += 1
...
def update_rank(event_id):
    ...
    selections = Selection.objects.filter(
        entry__event_id=event_id, entry__game__isnull=True
    )
    ...
    for game in games:
        selections = Selection.objects.filter(
            entry__event_id=event_id, entry__game_id=game.id
        )
        status = _update_game_rank(selections)
...
def get_entry_views(selections):
    ...
    for selection in selections:
        ...
        if method.startswith("KO") or "TKO" in method or "SUB" in method:
        ...
        if "DEC" not in _bout.method:
            ...
            if _bout.loser:
                if (
                    ...
                    selection.survivor2 and selection.survivor2.id == _bout.loser.id
                ):
...
def get_leaderboard_view(entries):
    ...
    leaderboard_views = sorted(
        leaderboard_views, reverse=True, key=lambda x: (x["coins"])
    )
...
class EntryViewSet(NestedViewSetMixin, viewsets.ModelViewSet):
    ...
    def get_contest_history_detail(self, request, **kwarg):
        try:
            ...
            if game_id == -1:
                ...
                selections = Selection.objects.filter(
                    entry__event_id=event_id, entry__game_id=game_id
                )
    ...
    def get_entries(self, request, **kwarg):
        try:
            ...
            data = []
            for entry in entries:
                data.append(dict(event=entry.event.name, user=entry.user.username))
    ...
    def bout_not_cancelled(self, _data, fighter):
        ...
        for bout in _data.get("bouts", []):
            if (
                fighter.id == bout["fighter1"]["id"]
                or fighter.id == bout["fighter2"]["id"]
            ):
                is_exist = True
    ...
    def add_fighters(self, game, event, data):
        ...
        if not bouts:
            bouts = BoutSerializer(
                Bout.objects.filter(event__id=event.id), many=True
            ).data
        for bout in bouts:
            bout["survivors"] = []
            bout["contests_orig"] = []
    ...
    def get_my_teams(self, request, **kwarg):
        ...
        selections = Selection.objects.filter(entry__user_id=request.user.id).filter(
            entry__last_edited__range=date_range
        )
        try:
            if request.user:
                latest_event = Event.objects.latest_event()
                ...
                for sel in selections:
                    ...
                    if event.id == latest_event.id:
                        ...
                        if is_new:
                            recent_bouts_dict = self.add_fighters(
                                game, event, recent_data
                            )
        ...
        live_data["teams"] = sorted(
            live_data["teams"],
            key=lambda x: (x["game"]["id"], x["game"]["entry_number"]),
        )
        recent_data["teams"] = sorted(
            recent_data["teams"],
            reverse=True,
            key=lambda x: (
                x["game"]["id"],
        ...
        return Response(
            dict(live_data=live_data, recent_data=recent_data), status=status
        )
    ...
    def withdraw_team(self, request, **kwarg):
        ...
        try:
            entries = Entry.objects.filter(
                ...
                entry_number=request.data["entry_number"],
            )
    ...
    def create(self, request):
        try:
            ...
            selection_serializer = SelectionSerializer(
                data=request.data["selections"], many=True
            )
            ...
            return Response(
                {"status": "failed", "message": "Something wrong happened."}, status=400
            )
            ...
            return Response(
                {
                    "status": "failed",
                    "message": "The # of entries reached out to the limit.",
                },
                status=403,
            )
        except Exception as err:
